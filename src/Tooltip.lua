...
local function calculateScore(link, loc, spec, tooltip, equippedItemHasUniqueGem)
    return ScoreModule:CalculateItemScore(link, loc, ScanningTooltipModule:ScanTooltip(link), spec, equippedItemHasUniqueGem);
end
    ...
    if IsEquippableItem(link) then
        ...
        for _, specKey in ipairs(Utils.OrderKeysBy(specs, "Order")) do
            ...
            if(spec.Enabled) then
                ...
                local score = calculateScore(link, loc, spec, tooltip, minEquippedScore and minEquippedScore.HasUniqueGem);
                ...
                if(not compact) then
                    ...
                    if(#upgrades ~= 0 and db.ShowUpgrades) then
                        for _, upgrade in ipairs(upgrades) do
                            local upgradeScore = calculateScore(upgrade.Link, loc, spec, tooltip, minEquippedScore and minEquippedScore.HasUniqueGem);
                            local upgradeDiff = 0;
